...
void Nodes::finish(){
    if(type == 3){
        ...
        for (int i = 0; i < arrayLength; i++)
            preArr[i] = 0;
//        fileInput.open(("F:/DowLoad/omnetpp-5.6.2-src-windows/omnetpp-5.6.2/samples/New folder/a/output.txt"), ios::in);
        if(fileInput){
            int i = 0;
        ...
        fileInput.close();
//        fileOutput.open(("F:/DowLoad/omnetpp-5.6.2-src-windows/omnetpp-5.6.2/samples/New folder/a/output.txt"), ios::out);
        for (int i = 0; i < arrayLength; i++) {
...
t++;
    ...
    if(getIndex() == 35){
    cout << "Received packets: "<< sum << endl;
    cout << "Thoughput: "<< (double)(sum * 100)/ (1*1000*1000*t*TIMEOUT) << endl;}
}
